...
def convert_disp_to_depth(disp,focal_length,baseline,scale=0):
    # Full resolution
    if scale ==0:
        baseline = baseline
    # 1/2 resolution
    ...
    depth = focal_length /(disp+1e-5)
    
    depth = torch.clamp(depth,min=0,max=400)
    depth = depth.float()
    
    return depth

def convert_to_free_trans(depth):
    trans =  torch.exp(-1.0*depth)
    return trans.float()
...
class DSSMMD(nn.Module):
    def __init__(self,in_channels=3,max_disp=192,dehaze_switch=False):
        ...
        if self.dehaze_switch:
            ...
            self.transmission_est3 = nn.Sequential(
                nn.Conv2d(64,1,kernel_size=3,stride=1,padding=1,bias=False),
                nn.Sigmoid())
        
            self.refine_trans2 = res_submodule_trans(value_planes=32,out_planes=32)
            self.refine_trans1 = res_submodule_trans(value_planes=32,out_planes=32)
            self.refine_trans0 = res_submodule_trans(value_planes=16,out_planes=16)
            ...
            # Airlight Estimation
            self.arilight_estimation = nn.Sequential(
                conv(3,16,7,2),
                ...
                nn.BatchNorm2d(64),
                nn.ReLU(True),
                nn.Conv2d(64,1,padding=0,kernel_size=1,bias=False),
                nn.Sigmoid()
    ...
    def forward(self,haze_left,haze_right,baseline,focal_length):
        
        # baseline shape:[B,1,1,1]
        # focal length : [B,1,1,1]
        ...
        disp3 = self.relu3(disp3)                    #[B,1,H//8,W//8]
        ...
        if self.dehaze_switch: 
            ...
            # predicted the transmissions at the 1/8 scales.
            pred_trans3 = self.transmission_est3(iconv3_trans)           
            
            # predict the transmissions at the 1/4 scales.
            ...
            # initial dispairty at 1/4 transmision map.
            pred_trans2 = F.interpolate(pred_trans3,scale_factor=2.0,mode='bilinear',align_corners=False)
            pred_trans2 = torch.clamp(pred_trans2,min=0,max=1.0)
            predicted_depth2 = convert_disp_to_depth(disp2,focal_length=focal_length,baseline=baseline,scale=2)
            predicted_free_trans2 = convert_to_free_trans(predicted_depth2)
            pred_trans2 = self.refine_trans2(predicted_free_trans2,pred_trans2,iconv2_trans)

            
            # predicted the transmission at the 1/2 scales.
            ...
            pred_trans1 = F.interpolate(pred_trans2,scale_factor=2.0,mode='bilinear',align_corners=False)
            pred_trans1 = torch.clamp(pred_trans1,min=0,max=1.0)
            predicted_depth1 = convert_disp_to_depth(disp1,focal_length=focal_length,baseline=baseline,scale=1)
            predicted_free_trans1 = convert_to_free_trans(predicted_depth1)
            pred_trans1 = self.refine_trans1(predicted_free_trans1,pred_trans1,iconv1_trans)
            ...
            # predicted the disparity at the full scales.
            pred_trans0 = F.interpolate(pred_trans1,scale_factor=2.0,mode='bilinear',align_corners=False)
            pred_trans0 = torch.clamp(pred_trans0,min=0,max=1.0)
            predicted_depth0 = convert_disp_to_depth(disp0,focal_length=focal_length,baseline=baseline,scale=0)
            predicted_free_trans0 = convert_to_free_trans(predicted_depth0)
            pred_trans0 = self.refine_trans0(predicted_free_trans0,pred_trans0,iconv0_trans)


            airlight = self.arilight_estimation(haze_left)
            airlight = torch.clamp(airlight,min=0.8,max=1.0)
            airlight = airlight.squeeze(-1).squeeze(-1) #[B,1]
        
            return [disp3,disp2,disp1,disp0],[pred_trans3,pred_trans2,pred_trans1,pred_trans0],airlight
...
if __name__=="__main__":
    ...
    focal_length = torch.abs(torch.randn(8,1,1,1)).cuda()
    baseline = torch.abs(torch.randn(8,1,1,1)).cuda()
    

    model = DSSMMD(in_channels=3,dehaze_switch=True).cuda()
    
    # Infernece
    disparity_pyramid, predicted_transmission,airlight =model(left_input,right_input,baseline,focal_length)
    ...
    # predicted transmission map.
    print_tensor_shape(predicted_transmission)
    print("----------------------")
    
    print_tensor_shape(airlight)
