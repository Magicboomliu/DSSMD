...
class DisparityTrainer(object):
    def __init__(self, args,
        ...
        self.devices = [int(item) for item in args.devices.split(',')]
        ngpu = len(args.devices)
        self.ngpu = ngpu
    ...
    def _prepare_dataset(self):
        ...
        if self.dataset == 'sceneflow':
            train_transform_list = [
                            complex_transforms.RandomCrop(320,400),
    ...
    def train_one_epoch(self, epoch, round,iterations,summary_writer,local_rank):
        ...
        for i_batch, sample_batched in enumerate(self.train_loader):
            clear_left = torch.autograd.Variable(sample_batched['clear_left_image'].cuda(local_rank), requires_grad=False)
            target_disp_left = torch.autograd.Variable(sample_batched['left_disp'].cuda(local_rank), requires_grad=False).unsqueeze(1)
            focal_length = torch.autograd.Variable(sample_batched['focal_length'].cuda(local_rank), requires_grad=False)
            baseline = torch.autograd.Variable(sample_batched['baseline'].cuda(local_rank), requires_grad=False)
            beta = torch.autograd.Variable(sample_batched['beta'].cuda(local_rank), requires_grad=False)
            airlight = torch.autograd.Variable(sample_batched['airlight'].cuda(local_rank), requires_grad=False)
