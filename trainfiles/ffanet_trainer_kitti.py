...
from dataloader.ComplexKITTILoader import ComplexKITTILoader
from dataloader import complex_transforms_kitti
...
# Keys 
complete_data=['clear_left','clear_right','gt_disp','left_pseudo_disp','right_pseudo_disp',
...
class DisparityTrainer(object):
    ...
    def _prepare_dataset(self):
        if self.dataset == 'KITTI':
            train_transform_list = [complex_transforms_kitti.RandomCrop(320,400),
                                    complex_transforms_kitti.RandomVerticalFlip(),
                            complex_transforms_kitti.ToTensor()
                            ]
            train_transform =complex_transforms_kitti.Compose(train_transform_list)

            val_transform_list = [complex_transforms_kitti.ToTensor()
                         ]
            val_transform = complex_transforms_kitti.Compose(val_transform_list)
            
            train_dataset = ComplexKITTILoader(data_dir=self.datapath,train_datalist=self.trainlist,test_datalist=self.vallist,
                                    dataset_name='SceneFlow',mode='train',transform=train_transform,visible_list=complete_data)
            test_dataset = ComplexKITTILoader(data_dir=self.datapath,train_datalist=self.trainlist,test_datalist=self.vallist,
    ...
    def train_one_epoch(self, epoch, round,iterations,summary_writer):
        ...
        for i_batch, sample_batched in enumerate(self.train_loader):
            clear_left = torch.autograd.Variable(sample_batched['clear_left'].cuda(), requires_grad=False)
            target_disp_left = torch.autograd.Variable(sample_batched['left_pseudo_disp'].cuda(), requires_grad=False).unsqueeze(1)
            ...
            target_disp_left[target_disp_left<=0] = 0.1
    ...
    def validate(self,summary_writer,epoch,vis=False):
        ...
        for i, sample_batched in enumerate(self.test_loader):
            ...
            target_disp_left_valid_mask = target_disp_left>0
            target_disp_left_valid_mask = target_disp_left_valid_mask.float()
            ...
            with torch.no_grad():
                ...
                dehaze_image = dehaze_image * target_disp_left_valid_mask
                clear_left = clear_left * target_disp_left_valid_mask
