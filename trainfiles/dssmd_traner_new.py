...
from models.DSSMD_new import DSSMMD
...
from losses.DSSMDLoss import Disparity_Loss,Airlight_Loss,RecoveredCleanImagesLoss,Transmission_Loss
import os
import time
from utils.metric import P1_metric,P1_Value,D1_metric,Disparity_EPE_Loss
from utils.visual import save_images,disp_error_img
...
class DisparityTrainer(object):
    ...
    def _set_loss_function(self):
        ...
        self.transmission_loss = Transmission_Loss(weights=[0.6,0.8,1.0,1.0])
    ...
    def train_one_epoch(self, epoch, round,iterations,summary_writer):
        ...
        for i_batch, sample_batched in enumerate(self.train_loader):
            ...
            if self.model =="DSSMD":
                ...
                disparity_pyramid, transmission_pyramid, predicted_airlight = self.net(haze_left,haze_right,baseline,focal_length)
                ...
                transmission_output = transmission_pyramid[-1]
               
                # Full scale dehaze left
                dehazed_left = RecoveredCleanFromTrans(transmission_map=transmission_output,
            ...
            transmission_loss = self.transmission_loss(transmission_pyramid,left_trans)
            # Predicted Airlight.
            ...
            recovered_loss = 0
            weights = [0.6,0.8,1.0,1.0]
            for cur_idx, pred_trans in enumerate(transmission_pyramid):
                recovered_loss_cur =self.recovered_left_loss(pred_trans,predicted_airlight,haze_left,clear_left)
                recovered_loss += recovered_loss_cur* weights[cur_idx]
            ...
            total_loss = disp_loss*2.0 + transmission_loss*1.0 + airlight_loss*1.0 + recovered_loss*1.0
    ...
    def validate(self,summary_writer,epoch,vis=False):
        ...
        for i, sample_batched in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                if self.model=="DSSMD":
                    disparity_pyrmaid,transmission_pyramid,pred_airlght = self.net(haze_left,haze_right,baseline,focal_length)
                    output = disparity_pyrmaid[-1]
                    pred_trans = transmission_pyramid[-1]
